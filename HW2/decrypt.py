...
class Number_handle:
    ...
    def int_to_binary(input, n):
        buffer = [0] * n
        for x in range(n):
            buffer[n - x - 1] = input % 2
            input = input // 2
        return buffer

    def Left_Rotation(a, off):
        return a[off:] + a[:off]
...
class Shift_operation:
    def initial_Permutation(text):
        ...
        return [text[x - 1] for x in ip_matrix]
    ...
    def final_Permutation(text):
        ...
        return [text[x - 1] for x in fp_matrix]
    ...
    def expand(text):
        ...
        return [text[x - 1] for x in expand_matrix]
    ...
    def permutation(text):
        ...
        return [text[x - 1] for x in p_matrix]

####################################################################################

def key_generate(key):
    l, r = Shift_operation.pc1(key)
    rotations = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]
    buffer = []

    for x in range(16):
        l = Number_handle.Left_Rotation(l, rotations[x])
        r = Number_handle.Left_Rotation(r, rotations[x])
        buffer.append(Shift_operation.pc2(l + r))
...
def s_box(text):
    s = [
                    ## s1~s8
                 [
                    4, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7, 0, 15,
                 ...
                 [
                     4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1, 13,
    ...
    text = [text[x:x + 6] for x in range(0, len(text), 6)]
    buffer = []

    for i in range(8):
        bits = text[i]
        r = s[i][Number_handle.binary_to_int([bits[0], bits[5], bits[1], bits[2], bits[3], bits[4]])]
        for bit in Number_handle.int_to_binary(r, 4):
            buffer.append(bit)

    return buffer

def feistel_Algorithm(text, key):
    return Shift_operation.permutation(s_box(Number_handle.binary_xor(Shift_operation.expand(text), key)))


def des_cycle(l, r, key):
    return r, Number_handle.binary_xor(l, feistel_Algorithm(r, key))


def hex_to_bin(input: str):

    try:
        input = format(int(input[2:], 16), '064b')
    except TypeError:
        print("Type Error, Please enter again --> Hex")
        sys.exit()

    output = [int(bit) for bit in input]
...
def des_decryption(ciphertext, key):
    ...
    m = Shift_operation.initial_Permutation(ciphertext)

    l, r = m[:len(m) // 2], m[len(m) // 2:]

    for i in range(16):
        l, r = des_cycle(l, r, sub_keys[i])

    return Number_handle.binary_to_int(Shift_operation.final_Permutation(r + l))
...
out = hex(des_decryption(input, key))
